...
static esp_err_t aht20_init_sensor(void) {
	...
	if ((status & 0x08) == 0) {
		...
		i2c_master_write_byte(cmd, 0xBE, ACK_CHECK_EN); // Init command
	...
	while (1) {
			...
			if (ret == ESP_OK) {
				...
				if ((data[0] & 0x80) == 0) {
					...
					uint32_t t = temperature * PRECISION;
					uint32_t h = humidity * PRECISION;

					ESP_LOGI(AHT20, "Temperature: %d.%d °C, Humidity: %d.%d %%", t / PRECISION, t % PRECISION, h / PRECISION, h % PRECISION);
				} else {
...
void app_main(void) {
  ...
  xTaskCreate(aht20_task, "aht20_task", 2048, NULL, 5, NULL);
}
